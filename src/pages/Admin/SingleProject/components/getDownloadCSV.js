...
export default function getDownloadCSV({ project }) {
    ...
    let csvData = project.shareholder.map((customer, mainKey) => {
        ...
        let privacyConsentFile = "";
        // if shareholder any submission has privacy consent file say 동의
        if (shareholderSubmissions?.some((sub) => sub.privacy_consent_file)) {
            privacyConsentFile = "동의";
        }
